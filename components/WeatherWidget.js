...
export default function WeatherWidget() {
  ...
  const fetchWeather = async (name) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/weather?district=${name}`);
  ...
  if (loading && !weather) {
    return (
      <div className="w-full h-64 bg-white/10 animate-pulse rounded-3xl border border-white/20 flex items-center justify-center">
        <span className="text-white/60 font-medium">Loading Live Weather...</span>
      </div>
    );
  }
  ...
  return (
    <div className="relative w-full group overflow-hidden bg-gradient-to-br from-white/15 to-white/5 backdrop-blur-xl border border-white/20 rounded-[2.5rem] shadow-2xl p-8 transition-all hover:shadow-green-900/20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
        <div>
          ...
          <p className="text-white/60 mt-1 font-medium">Agricultural Decision Support</p>
        </div>

        <div className="flex items-center gap-3 bg-white/10 p-1.5 pl-4 rounded-2xl border border-white/10">
          <span className="text-xs uppercase tracking-widest text-white/40 font-bold">District:</span>
          <select 
            value={district}
          ...
          >
            {weather?.districts?.map(d => (
              <option key={d.name} value={d.name} className="bg-green-900">{d.bn}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 mt-10 relative z-10">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="bg-white/10 px-2.5 py-1 rounded-lg border border-white/5 whitespace-nowrap">
                <span className="text-[10px] uppercase tracking-tighter text-white/40 block font-bold leading-none">Feels Like</span>
        ...
        <div className="lg:col-span-7 grid grid-cols-2 sm:grid-cols-3 gap-4">
          ...
          <WeatherStat icon="🌬️" label="Wind Speed" value={`${weather.wind} km/h`} />
          <WeatherStat icon="🌧️" label="Rain Chance" value={`${weather.rain_chance}%`} />
        </div>
      </div>
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div>
            <div className="text-lg font-bold text-white mb-1 uppercase tracking-wider flex items-center gap-2">
                Irrigation Status <span className="w-2 h-2 rounded-full bg-white animate-ping" />
            </div>
